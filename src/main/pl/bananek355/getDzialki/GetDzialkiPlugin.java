    ...
    private void showBorderForPlayer(Player player, PlotData plot) {
        ...
        for (int step = 0; step < 3; step++) {
            getServer().getScheduler().runTaskLater(this, () -> {
                ...
                for (int x = minX; x <= maxX; x++) {
                    player.spawnParticle(Particle.FLAME, x + 0.5, playerY + 0.5, minZ + 0.5, 1, 0, 0, 0, 0);
                    player.spawnParticle(Particle.FLAME, x + 0.5, playerY + 0.5, maxZ + 0.5, 1, 0, 0, 0, 0);
                }
                for (int z = minZ; z <= maxZ; z++) {
                    player.spawnParticle(Particle.FLAME, minX + 0.5, playerY + 0.5, z + 0.5, 1, 0, 0, 0, 0);
                    player.spawnParticle(Particle.FLAME, maxX + 0.5, playerY + 0.5, z + 0.5, 1, 0, 0, 0, 0);
                }
            }, step * 20L);
        }
    }
